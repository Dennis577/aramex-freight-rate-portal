...
```
_headers              ← Cloudflare 缓存控制规则（新增）
scripts/inject-env.sh ← 已更新，自动复制 _headers 到 dist/
netlify.toml         ← 保留，Netlify 停用后可读参考
```
...
点击 **Environment variables** → **Add variable**，添加以下三个：
...
| `ADMIN_PASSWORD_HASH` | `faffce77c92b11ccc8e5b2d7f12cfbde01a2ca50fd9b50d2b48494f27ae5ad4a` | 管理员密码 SHA-256 |

> ⚠️ `SUPABASE_ANON_KEY` 的值请从 Netlify Dashboard 复制，或从现有 `dist/assets/js/config.js` 中读取。
...
如果想继续使用 `aramex-rate.netlify.app` 或自己的域名：
...
### 方案 C：保留 Netlify 域名做重定向
在 Netlify 侧设置重定向，将旧域名指向新 Cloudflare 地址。
...
1. **Netlify 和 Cloudflare 并行运行**：
   - 迁移期间两个站点可同时在线
   - 确认 Cloudflare 完全正常后再停用 Netlify

2. **`_headers` 文件**：
   - 已加入仓库，Cloudflare Pages 构建时会自动读取 `dist/_headers`
   - 等效于 `netlify.toml` 中的 `[[headers]]` 配置
...
**Q：是否需要删除 netlify.toml？**
A：不需要。`netlify.toml` 只在 Netlify 构建时读取，Cloudflare Pages 会忽略它。保留它也不会有影响。
